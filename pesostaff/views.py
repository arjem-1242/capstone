...
def change_password_view(request):
    ...
    if request.method == "POST":
        form = PesoChangePasswordForm(request.POST)
        if form.is_valid():
            ...
            if not request.user.check_password(old_password):
                ...
                return redirect(reverse ("pesostaff:peso_staff_profile"))
    else:
        form = PesoChangePasswordForm()

    return render(request, "pesostaff/staff_dashboard.html", {"form": form})


@login_required
def peso_staff_profile(request):
    return render(request, 'pesostaff/profile.html')

def pesostaff_faqs(request):
    return render(request, 'pesostaff/faqs.html')
